...
- Each `-3` checkout carries a built graphify graph (`graphify-out/graph.json`). For a question about how code fits together, `graphify query "<question>" --graph <repo>/graphify-out/graph.json` is cheaper than grepping; after a pull the graph lags until someone runs `graphify update <repo>`, so trust `git show` over the graph for anything in this window's diff.
